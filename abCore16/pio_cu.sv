...
//        jmp_addr = instruction_data[4:0]; // For JMP
    ...
    assign push_instr = (opcode == `OC_PUSH);     // debug
//    assign pull_instr = (opcode == `OC_PULL);     // debug
    assign pull_instr = (opcode == 4'b1001);     // debug
    assign mov_instr  = (opcode[3:1] == 3'b101);  // debug
//    assign irq_instr  = (opcode == `OC_IRQ);      // debug
    assign irq_instr  = (opcode == 4'b1100);      // debug
...
//                    pc_write_en = 1'b1;      // TODO: instruction memory register
            ...
            EXECUTE: begin
                casez (opcode)
                    ...
                    `OC_MOV: begin // MOV instruction (NEW - PLACEHOLDER)
...
// ***************            
            DLY_S: begin
//                if (opcode == `OC_PULL && delay_counter == 0) begin
...
//    logic [3:0] decoded_instr;
//    assign decoded_instr = {jmp_instr, wait_instr, in_instr, out_instr, 
//                            push_instr, pull_instr, mov_instr, irq_instr, set_instr};
                            

//================================================================
// Debug: enum definition for PIO instructions
//================================================================
...
pio_instr_t debug_current_instr;

always_comb begin
    case (instruction_data[15:13])
        3'b000: debug_current_instr = JMP;
        3'b001: debug_current_instr = WAIT; 
        3'b010: debug_current_instr = IN;
        3'b011: debug_current_instr = OUT;
        3'b100: debug_current_instr = instruction_data[7] ? PULL : PUSH;
        3'b101: debug_current_instr = MOV;
        3'b110: debug_current_instr = IRQ;
        3'b111: debug_current_instr = SET;
        default: debug_current_instr = JMP; // Default to avoid X
    endcase
end
...
//    assign debug_pc = '0;
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            dbg_pio_cu <= { jmp_instr, wait_instr, in_instr, out_instr, push_instr, 
                            pull_instr, mov_instr, irq_instr, set_instr, current_state[2:0]};
       end
    end
