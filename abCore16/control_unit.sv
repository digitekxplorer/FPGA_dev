...
//                        reg_dest_addr_out = ir_operand1_reg[`REG_ADDR_WIDTH-1:0];
                ...
                case(ir_opcode_reg)
                    ...
                    `OP_MUL:  if(current_state==S_WRITEBACK) begin flags_write_en_out=1; reg_write_en_out=1; alu_op_out=`ALU_MUL; end
                    `OP_INC:  if(current_state==S_WRITEBACK) begin flags_write_en_out=1; reg_write_en_out=1; alu_op_out=`ALU_INC; end
                    `OP_DEC:  if(current_state==S_WRITEBACK) begin flags_write_en_out=1; reg_write_en_out=1; alu_op_out=`ALU_DEC; end
                    `OP_AND:  if(current_state==S_WRITEBACK) begin flags_write_en_out=1; reg_write_en_out=1; alu_op_out=`ALU_AND; end
                    `OP_OR:   if(current_state==S_WRITEBACK) begin flags_write_en_out=1; reg_write_en_out=1; alu_op_out=`ALU_OR; end
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (ir_opcode_load_en)   ir_opcode_reg   <= imem_rdata_i;
            if (ir_operand1_load_en) ir_operand1_reg <= imem_rdata_i;
            if (ir_operand2_load_en_dly) ir_operand2_reg <= imem_rdata_i;   // dly needed for BRAM latency
            if (ir_operand3_load_en_dly) ir_operand3_reg <= imem_rdata_i;   // dly needed for BRAM latency
        end
    end
